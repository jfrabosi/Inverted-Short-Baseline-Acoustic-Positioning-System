...
/**
 * @brief Class for storing information about platform kinematics
 */
...
public:
     ...
     */
    Hexapod_Kinematics(){};
    int8_t home(angle_t *servo_angles0, angle_t *servo_angles1, angle_t *servo_angles2, angle_t *servo_angles3);
    int8_t calcServoAngles(platform_t coord, angle_t *servo_angles, int platform_num);
    int8_t calcServoAnglesAlgo1(platform_t coord, angle_t *servo_angles, int platform_num);
    int8_t calcServoAnglesAlgo2(platform_t coord, angle_t *servo_angles, int platform_num);
    int8_t calcServoAnglesAlgo3(platform_t coord, angle_t *servo_angles, int platform_num);
    double getHX_X();
    double getHX_Y();
    double getHX_Z();
    double getHX_A();
    ...
    /**
     * NOTE: I (Jakob) modified the coordinate frame of each platform from the original implementation.
     * My coordinate frame is X forward (through a set of parallel servos), Y right, Z down. 
     * This will come up again in the Algo3 file. I believe the original impementation uses Y forward,
     * X left, and Z down.
     * 
